...
export async function GET(req:NextRequest){
  ...
  try{const ingest=await runPlanningIngestion('pending');const hubSync=await syncUnitedHubSource('planning',{since:new Date(Date.now()-48*60*60*1000).toISOString()});return NextResponse.json({ok:true,ingest,hubSync})}
