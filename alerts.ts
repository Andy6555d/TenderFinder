...
export async function sendPlanningAlerts() {
  ...
  if(!apiKey) return {sent:0,skipped:'RESEND_API_KEY not configured'}
  const admin=createAdminClient()
  // No geolocation involved - matches purely on category and score, the same shape as tender
  // alerts. Fetch broadly and let each member's own preferences filter below.
  ...
  const {data:profiles}=await admin.from('profiles').select('id,email,outlet_name,categories,min_relevance_score,notify_planning').eq('status','approved').eq('notify_planning',true)
  ...
  for(const p of profiles||[]){
    ...
    const candidates=(leads||[]).map((l:any)=>({...l,_kind:l.commencement_matched_at?'commencement':'new'}))
      .filter((l:any)=>!done.has(`${l.id}:${l._kind}`) && l.relevance_score>=(p.min_relevance_score||20) && (!p.categories?.length||l.categories?.some((c:string)=>p.categories.includes(c))))
      .sort((a:any,b:any)=>(a._kind==='commencement'?0:1)-(b._kind==='commencement'?0:1)||b.relevance_score-a.relevance_score)
      .slice(0,12)
    if(!candidates.length) continue
    const rows=candidates.map((l:any)=>`<li><strong>${escapeHtml(l.development_address||l.development_description?.slice(0,80)||'Planning lead')}</strong> — ${escapeHtml(l._kind==='commencement'?'commencement detected':l.project_stage.replaceAll('_',' '))} — score ${l.relevance_score}/100<br><a href="${process.env.NEXT_PUBLIC_SITE_URL}/planning/${l.id}">Open lead</a></li>`).join('')
    const res=await sendEmail(p.email,`${candidates.length} construction opportunit${candidates.length===1?'y':'ies'} matched`,`<p>Hello ${escapeHtml(p.outlet_name||'')},</p><p>New planning or commencement signals matched your categories:</p><ul>${rows}</ul><p>Planning categories and values are estimates. Verify the official planning/building-control record before acting.</p>`,apiKey)
